...
class ProductController extends Controller
{
    ...
    public function shop(Request $request)
    {
        ...
        $bestSellers = DB::table('elb_purchase_items')
            ->join('elb_purchases', 'elb_purchase_items.order_id', '=', 'elb_purchases.id')
            ->join('elb_items', 'elb_purchase_items.product_id', '=', 'elb_items.id')
            ->select('elb_items.category', 'elb_items.id', 'elb_items.name', 'elb_items.image_url', DB::raw('SUM(elb_purchase_items.quantity) as total_sold'))
            ->where('elb_purchases.order_date', 'like', "$month%")
            ->groupBy('elb_items.category', 'elb_items.id', 'elb_items.name', 'elb_items.image_url')
            ->orderBy('elb_items.category')
    ...
    public function submitReview(Request $request, Product $product)
    {
        ...
        $hasPurchased = \DB::table('elb_purchase_items')
            ->join('elb_purchases', 'elb_purchase_items.order_id', '=', 'elb_purchases.id')
            ->where('elb_purchase_items.product_id', $product->id)
            ->where('elb_purchases.user_id', $user->id)
            ->exists();
        if (!$hasPurchased) {
